 ...
 */
   ...
   //creates a skier list that is empty
   public SkierList() {
   ...
   //returns the the number of skiers in the list
   public int size() {
   	// Programmer note: The following is placeholder.
      return count;
   }
   
   //accesses and returns a skier in the list specified by their index position in the list
   public Skier get(int index) {	
         return skiers[index];
   }
   
   //removes a skier in the list specified by their index position in the list
   ...
   //adds a new skier to the list in no particular position in the list
   ...
   private void doubleArray(){
      Skier[] skiernew = new Skier[skiers.length*2];
      for(int i = 0; skiers.length>i; i++){
         skiernew[i]=skiers[i];
      }
      skiers = skiernew;
   }

   //determines if a skier is in the list
